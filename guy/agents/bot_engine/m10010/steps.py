...
from .state import (
    logger, _append_log, _get_equipment_reader, _get_service_call_writer,
)
...
def _llm_route_exits(step, session_data):
    """Use an LLM to decide which exit to take from an instructions node with exits.

    Sends the instructions text + current session fields to the model and asks it
